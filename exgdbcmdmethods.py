...
class ExgdbCmdMethods(object):
    ...
    def tracecontinue(self, *arg):
        ...
        r = re.compile(r"0x[0-9a-f]*\x1b\[0m\s\((.*)\)")
        r2 = re.compile(r"<.*>:\s*(.*)")
        r3 = re.compile(r"^([^\s]*)\s*(.*)")
        r_args = re.compile(r"(.*),(.*)")
        f = File(log_filename)
        ...
        asm_ops = {};
        def parse_arg(arg_str, text, f_log, f_symbol_memo):
            b = 0
            appending_one_log = ""
            arg_data_str = ""
            arg_reg_str = ""
        ...
        while True:
            ...
            asm_str = r.findall(text)
            #f.add("asm_str: " + asm_str + "\n")
            if len(asm_str) > 0:
                asm_str = asm_str[0]
            else:
                print("error")
                break
            if asm_str[0] == "<":
                asm_str = r2.findall(asm_str)
                if len(asm_str) > 0:
                    asm_str = asm_str[0]
            asm_str = r3.findall(asm_str)
            arg1 = ""
            arg2 = ""
            if len(asm_str) > 0:
                asm_op = asm_str[0][0]
